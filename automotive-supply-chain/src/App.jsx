...
import { ethers } from "ethers";
import "bootstrap/dist/css/bootstrap.min.css";
import abi from "/Users/sona/Desktop/Code/supply-chain/build/contracts/SupplyChain.json"; // Replace with correct ABI path
...
const App = () => {
  ...
  const CONTRACT_ADDRESS = "0x65bC9306d438c7a58bA7AFa3628496776b911F96"; // Replace with actual deployed address
  const CONTRACT_ABI = abi.abi;
  ...
  const genericAddEntity = async (e, methodName) => {
    e.preventDefault();
    clearMessages();
    const address = e.target.entityAddress.value;
    const name = e.target.entityName.value;
    const location = e.target.entityLocation.value;
    try {
      const tx = await contract[methodName](address, name, location);
      await tx.wait();
      setSuccess(`${methodName.replace("add", "")} added successfully!`);
    } catch (err) {
  ...
  const stageAction = async (fn, label) => {
    clearMessages();
    try {
      const tx = await contract[fn](partId);
      await tx.wait();
      setSuccess(`${label} completed!`);
    } catch (err) {
      setError(`Error in ${label.toLowerCase()}: ` + err.message);
    }
  };
  ...
  return (
    <div className="container my-5">
      ...
      {account && (
        <>
          {["Supplier", "Manufacturer", "Distributor", "Retailer"].map(
            (entity) => (
              <div key={entity}>
                <h3>Add {entity}</h3>
                <form
                  onSubmit={(e) => genericAddEntity(e, `add${entity}`)}
                  className="mb-4"
                >
                  <div className="mb-3">
                    <input
          ...
          <div className="mb-4">
            <button
              className="btn btn-primary me-2"
              onClick={() =>
                stageAction("componentSourcing", "Component Sourcing")
              }
            >
              Component Sourcing
            </button>
            <button
              className="btn btn-primary me-2"
              onClick={() => stageAction("Assembly", "Assembly")}
            >
              Assembly
            </button>
            <button
              className="btn btn-primary me-2"
              onClick={() => stageAction("qualityCheck", "Quality Check")}
            >
              Quality Check
            </button>
            <button
              className="btn btn-primary me-2"
              onClick={() => stageAction("delivery", "Delivery")}
            >
              Delivery
            </button>
            <button
              className="btn btn-primary"
              onClick={() => stageAction("retail", "Retail")}
            >
              Retail
            </button>
          </div>
          ...
          <h3>Check Part Stage</h3>
          <button className="btn btn-info mb-3" onClick={showStage}>
            Show Stage
          </button>
          {stage && (
            <p>
